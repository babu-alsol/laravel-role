                        ...
                        <form action="{{ route('admin.admins.update', $admin->id) }}" method="POST">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6 col-sm-12">
                                    ...
                                    <input value="{{ $admin->mobile }}" type="text" class="form-control" id="name"
                                        name="mobile" placeholder="Enter Name">
                                </div>
                                <div class="form-group col-md-6 col-sm-12">
                                    <label for="email">Employee Designation</label>
                                    <select name="designation_id" id="roles" class="form-control select2">
                                        @foreach ($designations as $item)
                                            <option {{ $item->id == $admin->designation_id ? 'selected' : '' }}
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6 col-sm-6">
                                    ...
                                    <select name="status" id="roles" class="form-control">
                                        <option {{ $admin->status == '1' ? 'selected' : '' }} value="1">Active
                                        </option>
                                        <option {{ $admin->status == '0' ? 'selected' : '' }} value="0">In Active
                                        </option>
                                    </select>
                                </div>
